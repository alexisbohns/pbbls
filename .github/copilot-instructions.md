...
6. **Lab Note (EN/FR)**: **required** for user-facing PRs. Gate: the PR has the `feat` label, **or** it touches a user-visible Arkaik view node (`docs/arkaik/bundle.json`). If gated in, author the note as the **YAML snippet** in the `## Lab Note (EN/FR)` section of the PR body — use the `lab-note` skill (`.claude/skills/lab-note/`) for the schema, allowed values, and tone (French uses "Tu"). Both languages are mandatory; no em dashes. Set `suggested.molecule: pbbls`. One block, two destinations: on merge `.github/workflows/lab-note.yml` posts it to the Ariko changelog vault automatically, and at release time a human pastes the same YAML into the Pebbles Lab admin. Never write to Supabase / `logs` from the dev loop. If the PR is not user-facing, delete the section entirely (add the `no-lab-note` label if the advisory reminder still comments).
