...
The diff is not byte-for-byte. `.github/scripts/normalize-db-types.py` strips the `__InternalSupabase` / `PostgrestVersion` block from **both** sides first: that block is emitted only when generating against a **linked** project, because the version is read from that project's API, so a `--local` generation can never reproduce it. It describes the server, not the schema. The committed file carries it because it was generated by the dashboard or the `generate_typescript_types` MCP tool rather than by `db:types`. Everything else is compared exactly — a renamed column, a dropped function or a new table survives the normalisation untouched. **Never widen that normaliser to make a red job pass**; a one-sided allowance is how a real drift hides.
