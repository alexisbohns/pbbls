...
- **D4 — Market reads are the three iOS queries verbatim; `buy_glyph` decodes the scalar jsonb directly.** Mine: `glyphs` + embedded `glyph_submissions(price, status, listed)`, price = first `approved && listed` submission else 0. Owned: `glyph_entitlements` with **no user filter** (RLS scopes), ordered by acquisition time, price = `price_paid`. Commu: `v_glyph_market` with server-side `.neq("user_id", me)` (view does NOT exclude own rows). Buy: `postgrest.rpc("buy_glyph", {p_glyph_id})` decoded straight to `BuyGlyphResult(entitlement_id, balance)` — never a single-row accessor. Error mapping = substring match on the lowercased message in iOS's order (`insufficient_karma` bubbles from `spend_karma`), copy verbatim from the iOS catalog: insufficient_karma → "Not enough karma", not_in_market → "This glyph is no longer available", already_owned → "You already own this glyph", cannot_buy_own → "This is your own glyph", fallback "Couldn't complete the swap. Please try again."; the list paths' missing-session copy is "Please sign in again.".
- **D5 — The swap's returned balance is applied to the shared stats directly; never a flash.** iOS assigns `stats.karma = result.balance` (server truth riding the RPC response — no refetch); Android mirrors with a narrow `PathStatsService.applyKarmaBalance(balance)` so the bar and Profile update instantly (M41 D2 sharing). The drawer morphs in place to its Owned state (no dismiss). Purchases never feed the karma flash (M39 D10).
