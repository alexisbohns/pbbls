...
**Architecture:** One new Theme file (`PebblesList.swift`) introducing `.pebblesList()`, `.pebblesListRow(position:)`, and `Text.pebblesSectionHeader()`. Each list/form in scope applies the modifier and decorates rows with a position-aware enum (`.only`/`.top`/`.middle`/`.bottom`). The border is drawn on a per-row overlay using `UnevenRoundedRectangle` because SwiftUI doesn't expose Section bounds.
