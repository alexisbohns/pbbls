...
- Ripple stack (D4): `features/shared/ripples/{RippleStrokeTone,RippleStrokes,RippleBadge}.kt` — pure tone truth table, six transcribed 44-viewBox paths, Canvas badge (outermost-first, opacities 0.33/0.66/1.0, 2dp round caps, centered digit, combined a11y label via `clearAndSetSemantics`).
