...
Pebbles est une application conçue pour vous aider à collecter et revisiter vos souvenirs quotidiens, en toute tranquillité. Nous croyons qu'une bonne privacy policy est avant tout un acte de respect envers nos utilisateurs. Ce document explique, de façon claire et transparente, quelles données nous collectons, pourquoi nous les collectons, comment nous les protégeons, et quels droits vous avez.

Pebbles fonctionne comme une application locale et personnelle. Nous ne profilons pas vos émotions pour la publicité. Nous ne partageons vos souvenirs nulle part. Vos données restent vôtres.
...
Lors de votre inscription et utilisation de Pebbles, vous nous fournissez volontairement les données suivantes :
...
Lors de l'utilisation de Pebbles, nous générons automatiquement :
...
Pour fonctionner et assurer votre sécurité, nous traitons :

- **Journaux d'authentification :** Les horodatages de vos inscriptions, connexions et déconnexions (via les journaux d'audit Supabase).
- **Tokens de session :** Les cookies de session fournis par Supabase pour maintenir votre connexion.
- **En-têtes HTTP standards :** Les métadonnées techniques de votre appareil et navigateur.
...
### 3.1 Exécution d'un Contrat (Art. 6(1)(b))

Le traitement des données suivantes est nécessaire à l'exécution du contrat d'utilisation de Pebbles :
- Données de compte (inscription, identification)
- Événements, humeurs et réponses (enregistrement et stockage)
- Bounce karma et achievements (fonctionnalité principale)
...
Nous utilisons vos données sur la base de notre intérêt légitime pour :
- **Sécurité :** Prévention des fraudes, authentification et maintien de la sécurité (journaux d'authentification).
- **Amélioration de produit :** Analyse des agrégats anonymisés pour améliorer l'expérience utilisateur.
...
Les humeurs, émotions et réflexions TCC que vous enregistrez dans Pebbles peuvent être qualifiées de données à caractère sensible au sens de l'article 9 du RGPD (données concernant la santé mentale et le bien-être psychologique).
...
- Aucun pixel de tracking ou outil de suivi.
...
Merci de faire confiance à Pebbles avec vos souvenirs.
