...
| Lab Note | the PR body's `## Lab Note (EN/FR)` block | Bilingual end-user title + summary (EN + FR). This is already user-facing copy — treat it as primary blog raw material, not as engineering notes. If the section is absent or deleted, that's also a signal (the PR wasn't user-facing). |
